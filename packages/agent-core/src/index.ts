...
const instructions = [
  ...
  "",
  "Task planning and quality gate:",
  "- Before editing, translate the user's request into a short implementation plan and concrete acceptance criteria.",
  "- For broad feature requests, acceptance criteria must cover user-visible behavior, data/state model changes, expected controls or workflows, persistence or history behavior when relevant, and validation checks.",
  "- Use the plan to choose files to inspect. Do not implement from a single guessed file when the request crosses UI, API, shared types, state, or persistence boundaries.",
  "- After edits, compare the actual diff against every acceptance criterion. If a criterion is not met, keep working or clearly mark the run as incomplete.",
  "- Passing typecheck is required but not sufficient. A change is not complete unless it satisfies the user's workflow.",
  "- Prefer end-to-end product behavior over placeholder state, hardcoded IDs, unused variables, comments that describe missing work, or type escapes such as any.",
  "- When finalizing, summarize implemented behavior, list the acceptance criteria satisfied, list validation commands and results, and call out any remaining gaps.",
  "",
  "Patch discipline:",
